...
class OrganizationManager(models.Manager):
    ...
    def creating_organization(self,form):
        error = []

        if len(form['org_name']) < 2:
            error.append("Organization name must be atleast 3 character(s)")
        ...
        if len(error) > 0:
            ...
            created=self.create(org_name=form['org_name'],first_name=form['first_name'], last_name=form['last_name'],address=form['address1'],city=form['city'], state=State.object.get(id=form['state']),email=form['email'], password=hash1)
            return created
...
class Organization(models.Model):
    ...
    state=models.ForeignKey(State,related_name="state_org")
